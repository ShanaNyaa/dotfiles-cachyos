...
- `tmux`'s truecolor override targets `xterm-ghostty` (Ghostty's real `TERM`), since SSH forwards `TERM` but not `COLORTERM`. Remote hosts need the matching terminfo entry installed before this works over SSH: `infocmp -x xterm-ghostty | ssh user@remote-host "tic -x -"`.
