...
-- Focus the window with the given class if one exists, otherwise launch cmd
local function focusOrLaunch(class, cmd)
	return function()
		local result = hl.dispatch(hl.dsp.focus({ window = "class:^(" .. class .. ")$" }))
...
hl.bind(mainMod .. " + T", focusOrLaunch(TERMINAL_CLASS, TERMINAL))
hl.bind(mainMod .. " + C", focusOrLaunch(CODE_CLASS, CODE))
